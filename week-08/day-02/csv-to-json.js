...
let row = data.split('\n').map(function(row) {
    let listOfAnimals = row.split(';');
    return {
        name: listOfAnimals[0],
        id: listOfAnimals[1],
        color: listOfAnimals[2],
    }
});

row = row.filter(function(line, i) {
    return row.slice(0,  i).every(element => element.name !== line.name);
})
